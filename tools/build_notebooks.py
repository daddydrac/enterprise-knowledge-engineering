...
def lesson(code,title,goal,explain,walks,task,answer,check,hint,reflection,track='enterprise',minutes=40):
    LESSONS.append(dict(code=code,title=title,goal=goal,explain=explain,walks=walks,task=task,answer=answer,check=check,hint=hint,reflection=reflection,track=track,minutes=minutes))

lesson('E00','Reproduce the source and the environment',
 'Verify that the learning graph comes from published clinical records and that every transformation is traceable.',
...
The core notebooks run offline after installing Python packages and Java 17. RDFLib handles graph storage and SPARQL; pySHACL checks explicit data contracts; OWLAPI checks profiles; HermiT handles OWL DL. Owlready2 supplies the packaged reasoner and JPype connects Python to Java. A pinned BFO core is included so imports resolve locally. The requirements-lock file records the environment used to execute the solutions.''',
 [('Inspect the manifest and verify the selected CSV', '''manifest=json.loads((ROOT/'data/source_manifest.json').read_text())
...
print({k:manifest[k] for k in ['kaggle_ref','kaggle_version','source_rows','subset_rows','source_coding']})'''),
  ('Read strings before casting only measured counts', '''import pandas as pd
...
assert set(frame.diag_1)=={'250.02','428','493'}''')],
 ...
 "sum(r['readmitted']=='<30' for r in rows())",'lambda x: x == 8','Read the category as a string; preserve the less-than sign.',
 'Explain why neither the cohort selection nor a code mapping establishes a causal claim about readmission.',minutes=30)

lesson('E01','Begin with a decision and a product owner',
 'Connect a competency question to a decision, evidence requirement and measurable adoption outcome.',
...
Do not fill a value case with unmeasured benefits. Begin with a baseline task observation, estimate a target separately, and record operating and review costs. Measure again with the product. A faster workflow that silently broadens a clinical category has not delivered the intended value.''',
 [('Write a decision contract', '''contract={
...
display(contract)'''),
  ('Separate measured data from future pilot measurements', '''measured_count=sum(r['diag_1']=='493' for r in rows())
...
print('Pilot measures remain uncollected:',contract['pilot_task_completion_rate'] is None)''')],
 ...
 "{'consumer':'Data analyst','decision':'Review a mapping with its evidence','baseline':None,'outcome_metric':'Correctly completed tasks / attempted tasks','adoption_metric':'Weekly returning users / eligible users'}",
 "lambda x: {'consumer','decision','baseline','outcome_metric','adoption_metric'} <= set(x) and x['baseline'] is None",
 'Specify both an answer-quality metric and a use metric; do not manufacture a baseline.',
 'Teach back the same inference to an engineer and a product owner, using one sentence for each.',minutes=35)

lesson('E02','Map records, encounters and people into RDF',
 'Construct triples while keeping real-world identity, record identity and source identifiers distinct.',
...
The adapter adds code-family types under an explicit local mapping policy. These are derived categorizations of code concepts, not new diagnoses of patients. The source CSV stays unchanged. A named graph can separate a transformation output from its source manifest, but graph names alone do not grant authorization or establish provenance. The pipeline must record who produced the graph and from which version.''',
 [('Build three triples by hand', '''r=rows()[0]
g=fresh_graph()
record=record_iri(r)
...
print(g.serialize(format='turtle'))'''),
  ('Inspect the complete adapter output', '''g=build_asserted()
...
assert len(members(g,EX.EncounterRecord))==60''')],
 ...
 "RES['patient/'+rows()[0]['patient_nbr']]", "lambda x: isinstance(x,URIRef) and str(x).endswith('/'+rows()[0]['patient_nbr'])",'Use RES and retain the patient identifier as text.',
 'What would break if a warehouse dimension surrogate key were treated as an enduring person identity?')

lesson('E03','Build a controlled vocabulary and a taxonomy',
 'Use SKOS labels, concept schemes and hierarchy without confusing concepts with OWL classes.',
...
Taxonomy quality includes more than syntax. Check preferred-label uniqueness, label-property overlap, roots, orphan concepts, unexpected cross-scheme edges and cycles. Confirm the meaning of a child is actually within its parent. Similar spelling, a shared department, or co-occurrence is not enough. OWL subclass relationships impose class-inclusion semantics; a SKOS broader edge does not automatically become a subclass axiom.''',
 [('Read the concept scheme', '''v=vocabulary()
display(list(query(v,'SELECT ?c ?label WHERE { ?c a skos:Concept; skos:prefLabel ?label } ORDER BY ?label')))
assert skos_issues(v)==[]'''),
  ('Observe an explicit hierarchy path', '''ancestors=list(query(v,'SELECT ?parent WHERE { <https://example.org/health/reporting/respiratory> skos:broader+ ?parent }'))
print(ancestors)
assert len(ancestors)==1''')],
 ...
 "(lambda g: (g.add((LOCAL.respiratory,SKOS.prefLabel,Literal('Respiratory grouping',lang='en'))),skos_issues(g))[1])(vocabulary())",
 "lambda x: any(i[0]=='preferred-label' for i in x)",'Call skos_issues after changing a copy. Do not edit the accepted file.',
 'When should a label be an alternative label instead of a new concept?')

lesson('E04','Align taxonomies across code systems',
 'Select a mapping relation, preserve its direction and scope, and keep proposals separate from accepted mappings.',
...
For each mapping record the two schemes, versions, source and target IRIs, relation direction, rationale, evidence, reviewer role, status and intended use. Evaluate precision on reviewed accepted links, coverage against the source scope, and unresolved cases separately. A confidence score is not an approval.''',
 [('Separate proposed from accepted statements', '''v=vocabulary()
...
display(list(query(v,'SELECT ?code ?family WHERE { ?code skos:broadMatch ?family } ORDER BY ?code')))'''),
  ('Read actual code reference files', '''import pandas as pd
...
assert not any(str(o).startswith(str(CM)) for o in build_asserted().objects(None,EX.primaryCode))''')],
 ...
 "len(list(query(vocabulary(),'SELECT ?source ?target WHERE { ?source skos:broadMatch ?target }')))",
 'lambda x: x == 3','The direction is source token -> broader reporting family.',
 'What evidence would you need before changing closeMatch to exactMatch? Explain the consequence of transitivity.',minutes=55)

lesson('E05','Separate TBox, ABox and RBox',
 'Recognize axioms about classes, individuals and relationships; explain why OWL is not a SQL constraint checker.',
...
An OWL domain or range says what follows when a property is used. It does not reject a row with an unexpected subject. Several domain statements apply together; they do not mean either domain is allowed. A necessary condition says every member must satisfy a description; it does not classify every thing satisfying that description. Equivalence supplies both directions. Check which direction your business requirement actually needs.''',
 [('Inspect axioms as RDF structures', '''s=schema()
...
print('Property chain heads:',list(s.objects(EX.aboutPatient,OWL.propertyChainAxiom)))'''),
  ('Compare asserted and classified memberships', '''asserted=build_asserted()
...
print('Entailed review candidates:',len(members(inferred,EX.ReviewCandidate)))''')],
 ...
 "{'subclass':'TBox','record_type':'ABox','property_chain':'RBox'}",
 "lambda x: x=={'subclass':'TBox','record_type':'ABox','property_chain':'RBox'}",'Focus on the subject of the logical statement, not its Turtle filename.',
 'Give one case where SHACL should reject data that remains consistent under OWL.')

lesson('E06','Align to BFO and define upper and lower bounds',
 'Align concrete entities to foundational categories and distinguish architectural layers from logical bounds.',
...
Keep approved bridges in a small domain module rather than editing BFO. Review scope, identity, time, disjointness, counterexamples, profile compliance and consistency before accepting a bridge. Never align an encounter record directly to process: the record describes a process but is not that process.''',
 [('Check actual BFO superclass entailments', '''report,inferred=reasoning()
...
print('Records classified as information content:',len(members(g,BFO.BFO_0000031)))'''),
  ('Compute the operational bounds over named source records', '''lower=members(g,EX.RespiratoryCodedRecord)
...
print({'lower':len(lower),'target':len(target),'upper':len(upper),'upper_not_known_target':len(upper-target)})''')],
 ...
 'BFO.BFO_0000015','lambda x: x == BFO.BFO_0000015','A hospital encounter unfolds through time.',
 'Why is a database row not the same entity as the event that row documents?',minutes=55)

lesson('E07','Run OWL DL and understand its limits',
 'Classify with HermiT, inspect profile reports, and distinguish graph answers from entailment.',
...
For an existential restriction, a reasoner may conclude that a suitable object exists without assigning it a public IRI. A missing documents edge can therefore be consistent in OWL while failing the SHACL requirement that the edge be supplied. Do not manufacture a named witness merely to make an export look complete.''',
 [('Read the profile and class results', '''report,inferred=reasoning()
...
assert len(members(inferred,EX.CardiopulmonaryRecord))==40'''),
  ('Remove a required explicit edge in a local experiment', '''changed=build_asserted()
...
assert dl_report['consistent'] and not validate(changed)[0]''')],
 ...
 'len(members(inferred,EX.CardiopulmonaryRecord))','lambda x: x == 40','Use the named class memberships returned by the reasoner.',
 'Why is absence from a SPARQL result not an OWL proof of negation?',minutes=55)

lesson('E08','Validate contracts with SHACL',
 'Separate intake validation, logical consistency and serving-graph checks.',
...
A serving shape may differ from an intake shape. If a consumer expects an inferred ReviewCandidate type, validate the explicit-plus-approved-derived graph. Name the target class and inference policy so the same data does not pass in one deployment and fail in another for an invisible reason. Validation reports are RDF and can be cataloged with the run's provenance and model version.''',
 [('Read the baseline validation report', '''conforms,report_graph,report_text=validate()
assert conforms
print(report_text)
print('Report triples:',len(report_graph))'''),
  ('Inject one missing identifier and locate its violation', '''bad=build_asserted()
...
print(text)''')],
 ...
 "(lambda g: (g.set((record_iri(rows()[0]),EX.stayDays,Literal(99,datatype=XSD.integer))),validate(g)[0])[1])(build_asserted())",
 'lambda x: x is False','Use Graph.set to replace the one value; 99 exceeds this cohort contract.',
 'Which version should change if the dataset expands to encounters longer than 14 days?')

lesson('E09','Bridge a warehouse and an ontology',
 'Reconcile SQL and SPARQL at the same grain, with explicit treatment of identifiers and missing values.',
...
Use reconciliation to discover hidden modeling changes. Compare the same population, filters, time window, units and code versions. A SQL count on source rows and a SPARQL count after a one-to-many label join can disagree even when neither parser reports an error. R2RML gives a standard mapping vocabulary; the supplied mapping file illustrates the subject template, logical table and predicate-object maps. The Python adapter is the executed implementation, not an R2RML processor.''',
 [('Ask equivalent SQL and SPARQL questions', '''db=sql_connection()
...
display(sql)'''),
  ('Inspect the declared R2RML bridge', '''print((ROOT/'ontology/encounter-mapping.ttl').read_text())
print('Physical key -> record IRI:',rows()[0]['encounter_id'],record_iri(rows()[0]))''')],
 ...
 "(sql_connection().execute('SELECT COUNT(*) FROM encounters WHERE time_in_hospital >= 7').fetchone()[0], int(list(query(build_asserted(),'SELECT (COUNT(?r) AS ?n) WHERE { ?r ex:stayDays ?d FILTER(?d >= 7) }'))[0][0]))",
 'lambda x: len(x)==2 and x[0]==x[1] and x[0]>0','Both counts must use encounters, not patients.',
 'How would a join to several alternate labels change a naive COUNT(?r)?',minutes=50)

lesson('E10','Query evidence across named graphs',
 'Publish a finite entailment view with source-level evidence and explicit graph scope.',
...
Named graphs are useful packaging boundaries, not a security system. Authorization must apply to evidence as well as answers. A graph query that reveals a hidden source identifier through an inferred label still discloses information. This teaching product uses public data; an enterprise deployment must implement the relevant access policy at the service and export layers.''',
 [('Build a dataset with separate inference output', '''report,inferred=reasoning()
ds=dataset(inferred)
...
display(answers[:5]);assert len(answers)==20'''),
  ('Check the empty default graph explicitly', '''assert len(list(query(ds,'SELECT ?s WHERE { ?s a ex:EncounterRecord }')))==0
print('20 review records have source evidence; default graph contains no intake records.')''')],
 ...
 '{str(row[0]) for row in answers}','lambda x: len(x)==20 and x=={r["encounter_id"] for r in rows() if r["diag_1"]=="493"}',
 'Use the graph-scoped evidence query; do not join by labels.',
 'What extra metadata would you need to reproduce the answer six months after a model update?')

lesson('E11','Extend, regress and retract an ontology',
 'Make a bounded extension and rebuild dependent conclusions after a change.',
...
OWL entailment is monotonic for a fixed language and growing premise set: adding axioms does not invalidate earlier logical consequences. Removing or correcting premises is different. A stored materialization can keep obsolete consequences unless it is retracted or rebuilt. This small course rebuilds from accepted source facts and the chosen model version; it does not pretend to implement incremental truth maintenance.''',
 [('Measure the extension impact', '''base_report,base=reasoning()
...
print({'before':len(before),'after':len(after),'newly_included':len(after-before)})'''),
  ('Delete a supporting fact and rebuild', '''edited=build_asserted()
...
print('Rebuilt named candidates:',len(members(rebuilt,EX.ReviewCandidate)))''')],
 ...
 'after-before','lambda x: len(x)==20 and all(xi.endswith(tuple(r["encounter_id"] for r in rows() if r["diag_1"]=="428")) for xi in x)',
 'Compare identities rather than only the two totals.',
 'Why should a rollout gate compare actual answer sets and not only their cardinality?',minutes=50)

lesson('E12','Catalog a semantic data product',
 'Package data, model versions, quality reports and service expectations for discovery and reuse.',
...
Publish source and ontology checksums, transformation version, reasoner configuration, accepted mapping set, validation result and run time with each release. Distinguish source vintage from refresh cadence: regularly rebuilding a dataset from 1999–2008 does not make the clinical observations current. Product adoption also depends on documentation, training and meaningful feedback channels; metadata completeness alone does not establish use.''',
 [('Create and query catalog metadata', '''cat=catalog()
...
assert len(list(cat.objects(product,DCAT.distribution)))==1'''),
  ('Inspect a product contract template', '''product_contract=json.loads((ROOT/'docs/product_contract.json').read_text())
...
assert product_contract['live_endpoint'] is None''')],
 ...
 'str(next(cat.objects(RES["distribution/encounters-v1"],EX.sha256)))',
 'lambda x: x==digest(ROOT/"data/encounters.csv")','Query the distribution metadata; use the actual file checksum.',
 'What should the product team do when a consumer reports that a correct answer is hard to interpret?')

lesson('E13','Deliver the capstone and measure adoption',
 'Deliver a working inference, evidence, a cataloged product and a pilot measurement plan.',
...
Your release review has four deliverables: working inference; explanation with source evidence; catalog entry and product contract; and a pilot scorecard covering correctness, task completion, review time, adoption and cost. A reviewer should be able to rerun the notebooks and explain one answer to a business stakeholder without reading the implementation.''',
 [('Run the release gates', '''assert validate()[0]
...
display(score)'''),
  ('Open the human-use scorecard', '''pilot=json.loads((ROOT/'docs/pilot_scorecard.json').read_text())
display(pilot)
assert pilot['measurements']['task_completion_rate'] is None''')],
 ...
 "{'publish_learning_artifact':True,'publish_clinical_service':False,'reason':'Local record-review learning product; clinical workflow and user pilot have not been validated.'}",
 "lambda x: x.get('publish_learning_artifact') is True and x.get('publish_clinical_service') is False and len(x.get('reason',''))>30",
 'A tested teaching product does not establish a tested clinical deployment.',
 'Present the four deliverables to a peer. Ask them to reproduce a source answer and identify one unsupported mapping.',minutes=70)

lesson('S00','Inspect the graph before writing a query',
 'Use small graph views, term inspection and basic SELECT patterns to understand the data.',
...
The original SPARQL course's 19 topics are retained through the topic crosswalk. The pwin course contributes a useful teaching rhythm: inspect a small graph, predict a result, run a query, explain the difference, and check expected values. We apply those techniques to the published healthcare subset. External browser tools and servers are optional; the reproducible core uses RDFLib locally.''',
 [('Inspect one subject', '''g=build_asserted();subject=record_iri(rows()[0])
display(list(g.predicate_objects(subject)))
display(list(query(g,'SELECT DISTINCT ?p WHERE { ?s ?p ?o } ORDER BY ?p')))'''),
  ('Run SELECT, ASK and CONSTRUCT', '''selected=list(query(g,'SELECT ?id WHERE { ?r ex:encounterId ?id } ORDER BY ?id LIMIT 5'))
...
display(selected)''')],
 ...
 "int(list(query(g,'SELECT (COUNT(?r) AS ?n) WHERE { ?r a ex:EncounterRecord }'))[0][0])",
 'lambda x: x==60','COUNT counts matched rows. Here each record has one explicit type match.',
 'Why does LIMIT without ORDER BY fail to define a repeatable first page?',track='sparql',minutes=30)

lesson('S01','Filter literals and bind values',
 'Use standard SPARQL expressions with explicit datatypes and safe parameter binding.',
...
display(result[:5])''')],
 ...
 "int(list(query(g,'SELECT (COUNT(?r) AS ?n) WHERE { ?r ex:a1cCategory \">8\" }'))[0][0])",
 'lambda x: x==13','The whole source category is the string >8, not a numeric laboratory value.',
 'When would lexical sorting of encounter IDs differ from numeric sorting?',track='sparql')

lesson('S02','Keep missing values and negation precise',
 'Predict OPTIONAL and FILTER behavior and distinguish MINUS from NOT EXISTS.',
...
The pwin optionality and negation exercises inspired these paired experiments. We use the real absence of A1C results in the selected source records to make the answer differences visible. Keep the original source token available for audit; lack of a result is not evidence of a normal value.''',
 [('Move one FILTER and compare row counts', '''g=build_asserted()
...
print({'filter_inside':len(inside),'filter_outside':len(outside)})'''),
  ('Compare negation with and without shared variables', '''missing=list(query(g,'SELECT ?r WHERE { ?r a ex:EncounterRecord FILTER NOT EXISTS { ?r ex:a1cCategory ?a } }'))
...
print({'missing_result':len(missing),'minus_without_shared_variables':len(minus)})''')],
 ...
 "len(list(query(g,'SELECT ?r WHERE { ?r a ex:EncounterRecord MINUS { ?r ex:a1cCategory ?a } }')))",
 'lambda x: x==42','Use the same record variable on both sides.',
 'What would happen if you changed the default graph to a union containing newer source records?',track='sparql',minutes=50)

lesson('S03','Aggregate at the correct grain',
 'Prevent duplicate joins and distinguish COUNT(*) from COUNT of a bound value.',
...
GROUP_CONCAT ordering is not portable unless a specific implementation contract guarantees it. SAMPLE chooses an arbitrary member. Do not use either as a deterministic evidence selection rule. A result that happens to stay stable during a small local run is not a specification guarantee.''',
 [('Count all rows and available values', '''g=build_asserted()
...
display(result)'''),
  ('See a cross-product inflate the count', '''inflated=list(query(g,'SELECT (COUNT(?r) AS ?n) (COUNT(DISTINCT ?r) AS ?unique) WHERE { ?r a ex:EncounterRecord . VALUES ?view { "source" "review" } }'))
...
display(inflated)''')],
 ...
 "{str(c).rsplit('/',1)[-1]:int(n) for c,n in query(g,'SELECT ?c (COUNT(DISTINCT ?r) AS ?n) WHERE { ?r ex:primaryCode ?c } GROUP BY ?c')}",
 "lambda x:x=={'250.02':20,'428':20,'493':20}",'Group by the code IRI; do not join labels before counting.',
 'How would you show zero counts for allowed categories with no encounters?',track='sparql')

lesson('S04','Walk taxonomy paths without claiming DL reasoning',
 'Use inverse, alternative and transitive paths while keeping reachability distinct from logical entailment.',
...
SKOS mapping paths need additional care. exactMatch is transitive by its semantics but closeMatch is not. Writing closeMatch+ asks a reachability question and does not make its endpoints a justified closeMatch. The pwin paths and inference lessons motivate this distinction; the revised course always declares whether it is navigating stored edges or asking a reasoner.''',
 [('Compare zero-or-more and one-or-more', '''v=vocabulary()
...
display(star)'''),
  ('Compare path matching with OWL classification', '''g=build_asserted()+schema()
...
print('Stored subclass path:',len(path_answer),'DL named classification:',len(members(derived,EX.ReviewCandidate)))''')],
 ...
 "list(query(vocabulary(),'SELECT ?child WHERE { <https://example.org/health/icd10cm/2026-04/J45> ^skos:broader ?child }'))[0][0]",
 'lambda x:x==CM["J45.909"]','The stored broader edge goes from the child to J45.',
 'Why would closeMatch+ be inappropriate as a rule for automatic code replacement?',track='sparql',minutes=50)

lesson('S05','Manage graphs and reversible updates',
 'Scope reads and updates explicitly, distinguish an empty graph from a deleted graph, and rebuild derived views.',
...
The local notebook demonstrates graph-scoped changes on a fresh in-memory dataset. The pwin graph-management exercises inform the sequence. Exported TriG preserves graph boundaries; Turtle does not represent a whole dataset's named graphs.''',
 [('Inspect named graph scope', '''ds=dataset()
...
assert len(list(query(ds,'SELECT ?r WHERE { ?r a ex:EncounterRecord }')))==0'''),
  ('Insert then clear an explicit work graph', '''work='urn:graph:work'
...
assert len(members(ds.graph(ASSERTED),EX.EncounterRecord))==60''')],
 ...
 "int(list(query(ds,'SELECT (COUNT(?r) AS ?n) WHERE { GRAPH <urn:graph:asserted:v1> { ?r a ex:EncounterRecord } }'))[0][0])",
 'lambda x:x==60','Name the graph in the query; the default graph is empty.',
 'What is the impact of changing a source graph while retaining yesterday\'s derived graph?',track='sparql')

lesson('S06','Query OWL DL results with a stated contract',
 'Distinguish ordinary SPARQL, exported entailments and a complete DL query regime.',
...
A useful query contract lists allowed graph patterns, ontology and data versions, import closure, consistency policy, reasoning engine and answer scope. Record expected positive and negative regression cases. If the ontology is inconsistent, stop serving its entailed answers. More triples do not make a contradictory model trustworthy.''',
 [('Compare an asserted query with a query over named entailed types', '''asserted=build_asserted()
report,derived=reasoning()
...
print(report['export_coverage'])'''),
  ('Show that export coverage excludes property-chain consequences', '''combined=asserted+derived
...
print('No aboutPatient triples are exported; that is a coverage limit, not a non-entailment claim.')''')],
 ...
 "{'coverage':'named_class_membership','consistency_required':True,'engine':'HermiT','query_engine':'RDFLib SPARQL 1.1 over the finite export'}",
 "lambda x:x.get('coverage')=='named_class_membership' and x.get('consistency_required') is True",
 'Be exact about what is materialized.',
 'What evidence would you require before claiming a service implements arbitrary OWL DL conjunctive queries?',track='sparql',minutes=55)

lesson('S07','Inspect blank nodes and OWL lists',
 'Read the RDF serialization of restrictions and lists without confusing that structure with its logical meaning.',
...
This lesson corrects an overstatement in the pwin inference commentary: anonymous union classes can be inspected and matched. It also retains the useful lesson that the domain of a property is an inference rule, not an input validation constraint. Keep list inspection, RDFS entailment and OWL DL interpretation conceptually separate.''',
 [('Traverse the union expression as RDF', '''g=schema()
...
display(union_members)'''),
  ('Round-trip without relying on blank-node labels', '''from rdflib.compare import isomorphic
...
print('Graph structure is preserved; blank-node spelling need not be.')''')],
 ...
 "[row[0] for row in query(schema(),'PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT ?p WHERE { ex:aboutPatient owl:propertyChainAxiom/rdf:rest*/rdf:first ?p }')]",
 'lambda x:set(x)=={EX.documents,EX.hasPatient}','This query retrieves members; use explicit rdf:first/rest positions if order matters.',
 'Why is a property chain ordered even though the set returned by a path query is not an ordered list?',track='sparql')

lesson('S08','Debug portability and application boundaries',
 'Separate syntax, data, semantics and implementation issues when a query behaves unexpectedly.',
...
Federation needs operational controls: an allowlist of destinations, timeouts, a clear policy on sending bound values, stable remote term identities and visible partial failures. SERVICE SILENT can preserve rows after a remote failure but can conceal incomplete enrichment. The course keeps network federation optional so credentials and endpoint availability do not block learning. The HTTP helper separates query, update and graph responses and raises failures instead of presenting them as empty answers.''',
 [('Parse standard syntax and detect a nonstandard form', '''from rdflib.plugins.sparql.parser import parseQuery
...
print('Portable query parsed; nonstandard LET rejected.')'''),
  ('Compare equivalent answer sets', '''g=build_asserted()
...
print('Equivalent named answers:',len(a))''')],
 ...
 "['syntax','dataset','terms','joins','entailment']",
 "lambda x:x==['syntax','dataset','terms','joins','entailment']",'Start with checks that can be resolved locally.',
 'Why is changing an HTTP failure into an empty result dangerous for a data product?',track='sparql',minutes=45)

lesson('S09','Build an explainable query product',
 'Assemble a reusable query, its expected identities, evidence and user-task acceptance test.',
...
An AI assistant may help draft a query or explain an answer, but the application still needs parsing, an allowed query surface, execution limits, authorized graph scope, evidence checks and human review of meaning changes. Treat retrieved content as data rather than instructions. Measure whether users can identify an unsupported mapping and reproduce an answer, not only whether the generated prose sounds convincing.''',
 [('Execute the final evidence query', '''report,derived=reasoning();ds=dataset(derived)
...
assert {str(x[0]) for x in result}=={r['encounter_id'] for r in rows() if r['diag_1']=='493'}'''),
  ('Attach a reproducible proof sketch', '''explanation={
...
display(explanation)''')],
 ...
 "'Find a review record, show its original source code, and explain why the proposed ICD-10-CM mapping is not yet accepted.'",
 "lambda x:isinstance(x,str) and 'source' in x.lower() and 'proposed' in x.lower()",
 'A usability task should test understanding of the evidence boundary.',
 'What would you change if users consistently confuse a review candidate with a confirmed diagnosis?',track='sparql',minutes=55)

lesson('E14','Use a small SWRL rule deliberately',
 'Write a positive relational rule, execute it on named source records, and choose when SWRL is appropriate.',
...
SWRL is not a general workflow engine. Standard rules are monotonic and do not provide ordinary closed-world negation-as-failure, deletion, arbitrary aggregate queries or safe external side effects. Built-ins such as arithmetic and comparison vary by reasoner. Do not assume the greaterThanOrEqual built-in works merely because a rule string parses. This tested rule uses no built-ins. After removing supporting facts, rebuild the rule ontology from source to avoid stale inferred properties.''',
 [('Declare the rule vocabulary and load real records', '''from owlready2 import World, Thing, ObjectProperty, Imp, sync_reasoner
world=World()
...
assert sum(len(x.respiratoryReviewFor) for x in EncounterRecord.instances())==0'''),
  ('Add the positive join rule and run its supported engine', '''with onto:
    rule=Imp()
    rule.set_as_rule(
...
display(sorted(actual)[:5])''')],
 ...
 "{'required_field':'SHACL','aggregate':'SPARQL','positive_join':'SWRL','class_inclusion':'OWL'}",
 "lambda x:x=={'required_field':'SHACL','aggregate':'SPARQL','positive_join':'SWRL','class_inclusion':'OWL'}",
 'The simplest mechanism with the required semantics is usually easiest to govern.',
 'Why does this positive rule not prove anything about records without a respiratory source code? What must be rebuilt when a supporting edge is removed?',minutes=50)

# A deliberately separate, small RL module; it is not the BFO imports closure.
next(x for x in LESSONS if x['code']=='E07')['walks'].append(
 ('Compare a restricted RL module', '''from owlrl import DeductiveClosure, OWLRL_Semantics
rl=Graph()
...
def make(spec,solved=False):
    cells=[nbf.v4.new_markdown_cell(f"# {spec['code']} · {spec['title']}\n\n**Outcome:** {spec['goal']}\n\n**Time:** about {spec['minutes']} minutes. Run cells in order. {'This is the executed solution edition.' if solved else 'Edit the exercise cell after completing the walkthrough.'}"),
           nbf.v4.new_markdown_cell(spec['explain']),nbf.v4.new_code_cell(BOOT)]
    for title,code in spec['walks']:
        cells += [nbf.v4.new_markdown_cell('## '+title),nbf.v4.new_code_cell(textwrap.dedent(code).strip())]
    cells += [nbf.v4.new_markdown_cell('## Your turn\n\n'+spec['task']+'\n\nReplace `answer = None` with your code. A skipped exercise is reported as incomplete; it is not a pass.'),
              nbf.v4.new_code_cell('answer = '+(spec['answer'] if solved else 'None  # Write your solution here'),metadata={'tags':['exercise']}),
              nbf.v4.new_code_cell(f"learner_check(answer, {spec['check']}, {spec['hint']!r})",metadata={'tags':['check']}),
              nbf.v4.new_markdown_cell('## Explain your model\n\n'+spec['reflection']+'\n\nWrite a short answer below. Check the relevant chapter in the book before promoting a model change.'),
              nbf.v4.new_markdown_cell('**My explanation:** '+('Compare the proof premises, source scope and query contract described above; use your own words in a peer review.' if solved else '_Write your explanation here._'))]
    return nbf.v4.new_notebook(cells=cells,metadata={'kernelspec':{'display_name':'Ontology Lab (Python 3)','language':'python','name':'ontology-lab'},'language_info':{'name':'python','version':'3.12'},'course':{'edition':'2026.1','track':spec['track'],'lesson':spec['code']}})

for spec in LESSONS:
    fname=spec['code']+'_'+spec['title'].lower().replace(' ','_').replace(',','').replace('?','')+'.ipynb'
    folder=ROOT/'notebooks'/spec['track'];folder.mkdir(parents=True,exist_ok=True)
    solutions=ROOT/'solutions'/spec['track'];solutions.mkdir(parents=True,exist_ok=True)
    nbf.write(make(spec),folder/fname)
    nbf.write(make(spec,True),solutions/fname)
    spec['notebook']=str((folder/fname).relative_to(ROOT))
(ROOT/'book/lessons.json').write_text(json.dumps(LESSONS,indent=2,ensure_ascii=False)+'\n')
print('Created',len(LESSONS),'learner notebooks and',len(LESSONS),'solution notebooks')
